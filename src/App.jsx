...
function App() {
  ...
  const addToCart = (selectedProduct) => {
    setCart((prevCart) => {
      const existingProduct = prevCart.items.find((item) => item.id === selectedProduct.id);

      if (existingProduct) {
        const updatedCart = prevCart.items.map((item) =>
          item.id === selectedProduct.id ? { ...item, qty: item.qty + 1 } : item
        );
        return { items: updatedCart, price: prevCart.price + selectedProduct.price };
      } else {
        const productToAdd = { ...selectedProduct, qty: 1 };
        return { items: [...prevCart.items, productToAdd], price: prevCart.price + selectedProduct.price };
      }
    });
  };
      ...
      <nav className="navbar navbar-light bg-light">
        <div className="container-fluid">
          <button
            ...
            onClick={toggleCart}
          >
            <h3>🛒 Cart {cart.items.length} | 💵 Rs.{cart.price}</h3>
          </button>
        </div>
      </nav>
